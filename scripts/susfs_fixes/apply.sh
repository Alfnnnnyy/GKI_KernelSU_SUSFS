...
# 兼容 Android 16 6.12: Linux 6.12 upstream 移除了 getname_flags 的第 3 个参数
if [[ "$ANDROID_VERSION" == "android16" && "$KERNEL_VERSION" == "6.12" ]] \
  && grep -qF 'getname_flags(filename, lookup_flags, NULL)' "$SUSFS_PATCH"; then
  echo "检测到 Android 16 6.12 getname_flags 双参数接口，修正 SUSFS 补丁中的三参数调用"
  sed -i 's/getname_flags(filename, lookup_flags, NULL)/getname_flags(filename, lookup_flags)/g' "$SUSFS_PATCH"
fi
